...
- `npm run solve` — level solver/validator CLI (`scripts/solve.ts`, via `vite-node`); see "Level solver" below. Exits non-zero if any level has unreachable characters, unreachable items, timeline anachronisms, or fails to load.
...
A level is an authored Markdown file in `public/levels/`. The available levels are indexed by `public/levels/levels.md` (currently `00_prologue.md`, `01_birth_of_constantine.md`, `02_house_of_rocks.md`); shared `characters.md` and `items.md` are pulled into individual levels by the import mechanism in [src/levelLoading/levelImportUtil.ts](src/levelLoading/levelImportUtil.ts) (cycle- and duplicate-aware, with per-line source tracking for error messages). The Markdown is the source of truth for rooms, characters, items, an itinerary, and conclusions. Loading is orchestrated by [src/levelLoading/levelUtil.ts](src/levelLoading/levelUtil.ts):
...
- [ADR (Solver)](docs/adr-solver.md) — Level solver. Characters are nodes; an undirected edge means two characters shared a room at the same time (co-presence, recomputed by sampling at level start + every `ROOM_ENTRY` tick + the timeline end — *not* reused from `CharacterEncounterEvent`s, which skip start-of-level co-presence; the timeline-end sample is required or a tour's final room is never observed). Edge model is future-proofed for directed (hidden-actor) edges. Validates **three** things: every character reachable from the active character, every placed item reachable (a reachable character is co-present with it), and no **timeline anachronisms** (a character scheduled into two overlapping same-channel activities — e.g. an absolute arrival back-planned over an earlier one). All must pass. Read before changing `src/solver/`.
