...
- [ADR 007](docs/adr-007-timeline-start-end-config.md) — Timeline start and end configuration. The `# general` section accepts `startTime` (preferred) and `endTime`; `time` remains a legacy alias for `startTime`. When `endTime <= startTime` the level crosses midnight and absolute itinerary timestamps less than `startTime` auto-shift by 24h, so authors keep using natural `HH:MM:SS` notation through midnight rollover. When `endTime` is explicit, the loader rejects itinerary events outside `[startTime, endTime]`. `Level.endTime` and `Level.duration` are both exposed and satisfy `endTime === startTime + duration`.
