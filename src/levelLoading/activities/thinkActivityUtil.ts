...
function _createOverlappingThoughtMessage(overlappingThoughtEvent:ThoughtEvent, thoughtEvent:ThoughtEvent, timestampKind:ActivityContext['timestampKind']):string {
...
export function tryCreateThinkActivity(activityText:string, context:ActivityContext):ItineraryEvent[]|null {
  ...
  if (overlappingThoughtEvent) throw new Error(_createOverlappingThoughtMessage(overlappingThoughtEvent, thoughtEvent, context.timestampKind));
  return [thoughtEvent];
}
