/* This module groups standing, sitting, and laying activity parsing for itinerary loading.
...
type BodyOrientationVerb = 'stands' | 'sits' | 'lays';
...
export function tryCreateBodyOrientationActivity(activityText:string, context:ActivityContext):ItineraryEvent[]|null {
  const verb = findSentenceStyleActivityVerb(activityText, ['stands', 'sits', 'lays'] as const);
  if (!verb) return null;
